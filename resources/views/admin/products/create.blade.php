...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4 text-white">Добавить новый товар</h1>
        ...
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-4">
                ...
                <input type="number"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="price" name="price" min="0" step="0.01" required value="{{ old('price') }}">
                @error('price')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select
                    ...
                    <option value="1" {{ old('in_stock', true) ? 'selected' : '' }}>Да</option>
                    <option value="0" {{ old('in_stock', false) ? 'selected' : '' }}>Нет</option>
                </select>
                @error('in_stock')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <select name="size[]" id="size" multiple
                    ...
                    @foreach($availableSizes as $sizeOption)
                        <option value="{{ $sizeOption }}" {{ in_array($sizeOption, old('size', [])) ? 'selected' : '' }}>
                            {{ $sizeOption }}
                        </option>
                    @endforeach
                </select>
